 ...
 *
 * This is the single source of truth for "AI without a key": it inspects the prompt and
 * returns sensible, structured responses (diet plans, workout plans, recipes, coach chat, etc.)
 * so the entire product is fully usable before any provider credentials are added. The Mock
 * provider wraps this, and the registry falls back to it when no live provider succeeds.
 *
 * Ported from the original lib/ai.ts fallback so behavior is unchanged.
 */

interface ScalableExercise {
  name?: string;
  sets?: number;
  reps?: string | number;
  rest_seconds?: number;
...
export function deterministicAIResponse(prompt: string): string {
  const promptLower = prompt.toLowerCase();

  // 0. MEAL PHOTO ANALYSIS (multimodal) — matched first via its unique marker.
  if (promptLower.includes('analyze this meal photo')) {
    return JSON.stringify({
      items: ['Mixed plate (estimated)', 'Grains', 'Protein', 'Vegetables'],
      calories: 520,
      protein_g: 28,
      carbs_g: 55,
      fat_g: 18,
      healthierAlternative:
        'Swap refined carbs for whole grains and add a side of greens to boost fiber and fullness.',
      confidence: 'low',
    });
  }

  // 1. DIET PLAN GENERATION (checked first so 7-day prompts don't match recipe/chat below)
  if (
    ...
    for (let i = 0; i < 7; i++) {
      mealPlan.push({
        day: weekdays[i],
        breakfast: { name: breakfastList[i % breakfastList.length], calories: Math.round(targetCals * 0.25), protein_g: dietType === 'non-veg' ? 25 : 15, carbs_g: 40, fat_g: 10 },
        lunch: { name: lunchList[i % lunchList.length], calories: Math.round(targetCals * 0.35), protein_g: dietType === 'non-veg' ? 35 : 20, carbs_g: 55, fat_g: 12 },
        dinner: { name: dinnerList[i % dinnerList.length], calories: Math.round(targetCals * 0.25), protein_g: dietType === 'non-veg' ? 30 : 18, carbs_g: 35, fat_g: 8 },
        snacks: { name: snacksList[i % snacksList.length], calories: Math.round(targetCals * 0.15), protein_g: 10, carbs_g: 15, fat_g: 6 },
      });
    }
    return JSON.stringify(mealPlan);
  }

  // 2. FRIDGE RECIPE GENERATION
  ...
  if (promptLower.includes('generate a') && (promptLower.includes('workout') || promptLower.includes('training'))) {
    ...
    const list = exercisesLibrary[equip] || exercisesLibrary.gym;
    const plan: Array<{ day: string; focus?: string; exercises: ScalableExercise[] }> = [];

    for (let d = 1; d <= days; d++) {
      const dayExercises: ScalableExercise[] = [];
      ...
      for (let e = 0; e < numExercises; e++) {
        ...
        dayExercises.push({ name: item.name, sets, reps, rest_seconds: rest, muscle_group: item.muscle, tip: item.tip });
      }
      plan.push({ day: `Day ${d}`, exercises: dayExercises });
    }
    return JSON.stringify(plan);
  }

  // 4. EXERCISE SWAP ALTERNATIVES
  if (promptLower.includes('biomechanically') || promptLower.includes('swap') || promptLower.includes('alternative')) {
    const isHinglish = promptLower.includes('hinglish');
    const alternatives = [
      { name: 'Dumbbell Press Alternative', sets: 3, reps: '10-12', rest_seconds: 75, type: 'Free Weight', tip: isHinglish ? 'Stabilizer muscles target honge. Spine ko flat aur core tight rakhein.' : 'Targets stabilizer muscles. Keep your spine flat and engage your core.' },
      { name: 'Cable Chest Press / Machine Press', sets: 3, reps: '12', rest_seconds: 60, type: 'Machine/Cable', tip: isHinglish ? 'Constant resistance aur mechanical tension hold karne par dhyan dein.' : 'Focus on constant mechanical tension and controlling the eccentric phase.' },
      { name: 'Deficit Push-ups (Joint Friendly)', sets: 3, reps: 'Max-1', rest_seconds: 45, type: 'Bodyweight/Joint-Friendly', tip: isHinglish ? 'Apni joints par load kam rakhne ke liye slow negatives perform karein.' : 'Perform slow negative counts to drop load stress on elbow/shoulder joints.' },
    ];
    return JSON.stringify(alternatives);
  }

  // 5. WORKOUT INTENSITY AUTO-PROGRESSION
  if (promptLower.includes('progressive overload') || promptLower.includes('challenging') || promptLower.includes('scale')) {
    ...
    try {
      ...
      if (match) {
        const list = JSON.parse(match[1]) as ScalableExercise[];
        const progressed = list.map((ex) => {
          let reps = ex.reps;
          if (typeof reps === 'number') reps = reps + 2;
          else if (typeof reps === 'string') {
            const parts = reps.split('-');
            reps = parts.length === 2 ? `${parseInt(parts[0]) + 2}-${parseInt(parts[1]) + 2}` : `${reps} + 2 reps`;
          }
          const baseTip = ex.tip || '';
          return { ...ex, reps, tip: baseTip + (isHinglish ? ' (Intensity Progressed: Negative movement ko slow karein)' : ' (Intensity Progressed: Focus on slow negatives for extra tension)') };
        });
