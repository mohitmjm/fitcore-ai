...
export async function ensureIndexes(db: Db): Promise<void> {
  await Promise.all([
    ...
    // One trust-based, server-approved Momentum Quest completion per local calendar day.
    db.collection('momentum_quest_completions').createIndex(
      { clerkUserId: 1, date: 1 },
      { name: 'user_day', unique: true },
    ),
