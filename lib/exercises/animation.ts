...
export interface ExerciseAnimation { id: string; slug: string; posture: ExercisePosture; duration: number; phases: string[]; primaryMuscles: MuscleId[]; secondaryMuscles: MuscleId[]; equipment: EquipmentId[]; frames: Array<{ progress: number; pose: JointPose }>; reducedMotionFrames: JointPose[]; isometric?: boolean; }
...
const define = (slug: string, duration: number, phases: string[], primaryMuscles: MuscleId[], secondaryMuscles: MuscleId[], equipment: EquipmentId[], active: JointPose, setup: JointPose = stand, isometric = false, posture: ExercisePosture = 'standing'): ExerciseAnimation => ({ id: `svg-${slug}`, slug, posture, duration, phases, primaryMuscles, secondaryMuscles, equipment, isometric, frames: [{ progress: 0, pose: setup }, { progress: .42, pose: active }, { progress: .55, pose: active }, { progress: 1, pose: setup }], reducedMotionFrames: [setup, active] });
const floor = (pose: JointPose): JointPose => ({ ...stand, ...pose });

export const EXERCISE_ANIMATIONS: Record<string, ExerciseAnimation> = {
  'incline-dumbbell-press': define('incline-dumbbell-press', 2900, ['Set on bench', 'Lower', 'Press inward', 'Lock out'], ['upper-chest', 'chest'], ['front-deltoids', 'triceps'], ['incline-bench', 'dumbbells'], { torso: 4, pelvis: 2, leftUpperArm: -76, rightUpperArm: 76, leftForearm: -34, rightForearm: 34 }, stand, false, 'incline'),
  'push-up': define('push-up', 2600, ['Plank', 'Lower together', 'Press floor', 'Finish'], ['chest'], ['triceps', 'front-deltoids', 'upper-abs'], ['floor'], floor({ leftUpperArm: 92, rightUpperArm: 92, leftForearm: 44, rightForearm: 44, leftThigh: 4, rightThigh: 4 }), stand, false, 'prone'),
  'seated-dumbbell-shoulder-press': define('seated-dumbbell-shoulder-press', 2700, ['Seated set', 'Press overhead', 'Pause', 'Lower'], ['shoulders', 'front-deltoids'], ['side-deltoids', 'triceps'], ['upright-bench', 'dumbbells'], { leftUpperArm: -150, rightUpperArm: 150, leftForearm: -8, rightForearm: 8, leftThigh: 8, rightThigh: -8 }, stand, false, 'seated'),
  ...
  'seated-wrist-curl': define('seated-wrist-curl', 2200, ['Forearms set', 'Lower wrist', 'Curl wrist', 'Reset'], ['forearms'], [], ['bench', 'dumbbells'], { torso: 16, leftUpperArm: -72, rightUpperArm: 72, leftForearm: 42, rightForearm: -42 }, stand, false, 'seated'),
  'dead-bug': define('dead-bug', 3100, ['Brace', 'Opposite reach', 'Hold', 'Return'], ['upper-abs', 'lower-abs'], ['hip-flexors'], ['mat'], floor({ leftUpperArm: -128, rightUpperArm: 8, leftThigh: -132, rightThigh: 82 }), stand, false, 'supine'),
  'reverse-crunch': define('reverse-crunch', 2700, ['Set knees', 'Pelvic curl', 'Small lift', 'Lower'], ['lower-abs'], ['upper-abs', 'hip-flexors'], ['mat'], floor({ pelvis: -22, leftThigh: -112, rightThigh: 112, leftShin: 35, rightShin: -35 }), stand, false, 'supine'),
  'side-plank': define('side-plank', 3600, ['Elbow set', 'Lift hips', 'Breathe', 'Hold'], ['obliques'], ['abductors', 'shoulders'], ['mat'], floor({ torso: -18, pelvis: -18, leftThigh: 4, rightThigh: -4 }), floor({ torso: -12, pelvis: -12, leftThigh: 4, rightThigh: -4 }), true, 'side'),
  ...
  'one-arm-dumbbell-row': define('one-arm-dumbbell-row', 2700, ['Bench support', 'Hang', 'Row elbow', 'Lower'], ['lats', 'upper-back'], ['biceps', 'rear-deltoids'], ['bench', 'dumbbell'], { torso: 8, leftUpperArm: -72, rightUpperArm: 118, rightForearm: -72, leftThigh: 8, rightThigh: -8 }, stand, false, 'row'),
  'neutral-grip-lat-pulldown': define('neutral-grip-lat-pulldown', 2800, ['Overhead stretch', 'Pull elbows', 'Upper chest', 'Return'], ['lats'], ['biceps', 'upper-back'], ['cable-machine'], { leftUpperArm: -116, rightUpperArm: 116, leftForearm: 30, rightForearm: -30, leftThigh: 8, rightThigh: -8 }, stand, false, 'seated'),
  'bird-dog': define('bird-dog', 3100, ['Quadruped', 'Opposite reach', 'Hold level', 'Return'], ['lower-back'], ['glutes', 'upper-abs'], ['mat'], floor({ leftUpperArm: -118, rightUpperArm: 35, leftThigh: -30, rightThigh: 118, rightShin: -18 }), stand, false, 'quadruped'),
  'hip-thrust': define('hip-thrust', 2800, ['Bench set', 'Drive hips', 'Glute squeeze', 'Lower'], ['glutes'], ['hamstrings', 'lower-back'], ['bench', 'barbell'], { torso: 5, pelvis: 12, leftThigh: 48, rightThigh: -48, leftShin: -42, rightShin: 42 }, stand, false, 'hip-thrust'),
